...
CollarLineOdomNode::CollarLineOdomNode(ros::NodeHandle& private_nh) :
    cumulated_transformation_(Eigen::Matrix4f::Identity()), b_save_file_(false), msg_count_(0), frame_id_("odom"), child_frame_id_("") {
    ...
    private_nh.param("frame_id", frame_id_, frame_id_);
    private_nh.param("child_frame_id", child_frame_id_, child_frame_id_);
...
void CollarLineOdomNode::msgCallback(const sensor_msgs::PointCloud2::ConstPtr& input_cloud) {
    ...
    if (child_frame_id_.empty()) {
        odom_msg.child_frame_id = input_cloud->header.frame_id;
    } else {
